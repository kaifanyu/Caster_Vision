...
def _pipeline_kwargs(config: Mapping[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "track_config": dict(track) if isinstance(track, Mapping) else {},
        "estimate_config": dict(estimate) if isinstance(estimate, Mapping) else {},
    }
...
def _stage_a() -> dict[str, Any]:
    ...
    command = [
        ...
        "-m",
        "pytest",
        str(PROJECT_ROOT / "tests" / "test_units.py"),
        "-q",
    ]
    ...
    return {
        ...
        "gate": "tests/test_units.py exits 0",
    }
...
def run_validation(
    ...
    *,
    ...
    destination = (
        ...
        else (resolved_config.parent / "out" / "synthetic").resolve()
    )
