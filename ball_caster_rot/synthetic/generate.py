...
on each hemisphere, each hemisphere follows ``Rx(alpha) @ Rz(beta)``, and the
same pinhole geometry consumed by the measurement pipeline projects them.  All
angles written to ``ground_truth.json`` are radians.
...
or as a command line utility::
    ...
    python synthetic/generate.py --output out/synthetic/default
    python synthetic/generate.py --output out/synthetic/calibration \
...
class Trajectory:
    ...
    def __post_init__(self) -> None:
        ...
        gamma_top = (
            np.zeros_like(alpha)
            if self.gamma_top is None
            else np.asarray(self.gamma_top, dtype=np.float64).reshape(-1)
        )
        gamma_bottom = (
            np.zeros_like(alpha)
...
def _motion_matrix(alpha: float, beta: float, gamma: float = 0.0) -> Array:
    """Ball-frame orientation for the caster convention.

    With ``gamma == 0`` this is the modeled ``Rx(alpha) @ Rz(beta)`` manifold.
    A non-zero ``gamma`` reproduces the intrinsic ``XYZ`` triple that
    :func:`ballrot.rotation.decompose_alpha_beta` returns.
    """

    return _rx(float(alpha)) @ _ry(float(gamma)) @ _rz(float(beta))
