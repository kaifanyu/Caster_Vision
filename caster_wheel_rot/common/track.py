...
def track_features(
    ...
    *,
    ...
    if initial_uv_curr is not None:
        initial = np.asarray(initial_uv_curr, dtype=np.float32)
        if initial.shape != points.shape or not np.isfinite(initial).all():
            raise ValueError("initial_uv_curr must match finite uv_prev")
        initial = initial.reshape(-1, 1, 2).copy()
    forward, status_forward, _ = cv2.calcOpticalFlowPyrLK(
        previous, current, points.reshape(-1, 1, 2), initial,
        flags=cv2.OPTFLOW_USE_INITIAL_FLOW if initial is not None else 0, **lk_params
    )
